...
{    
    ...
    static std::mutex globalPartition;
    
    static std::array<std::vector<Row>, HashTable_phase1<CACHE_SIZE>::partition_size> globalPartitions;
    ...
    void flush()
    {
        HashTable_phase1<CACHE_SIZE>::globalPartition.lock();
        for(auto entry : this->data)
        {
            HashTable_phase1<CACHE_SIZE>::globalPartitions[getIndex(hash(entry.key))].emplace_back(entry);
        }
        HashTable_phase1<CACHE_SIZE>::globalPartition.unlock();
...
public:
    ...
    struct Phase2
    {
        void operator()(const tbb::blocked_range<int> &r) const
        {
            ...
            for(int i = r.begin(); i != r.end(); ++i)
            {
                ...
                for(auto currEntry = HashTable_phase1<CACHE_SIZE>::globalPartitions[i].begin(); currEntry != HashTable_phase1<CACHE_SIZE>::globalPartitions[i].end(); ++currEntry)
                {
                    my_worker.localHashTable_phase2.insert(currEntry->key, currEntry->value);
                }
                HashTable_phase1<CACHE_SIZE>::globalPartitions[i].clear();
    ...
    void parallelGroup(Relation const& relation)
    {
        ...
        *  
        */
        tbb::parallel_for(tbb::blocked_range<int>(0, HashTable_phase2<CACHE_SIZE>::max_elements), Phase2());
...
template<int CACHE_SIZE>
std::array<std::vector<Row>, HashTable_phase1<CACHE_SIZE>::partition_size> HashTable_phase1<CACHE_SIZE>::globalPartitions;
    
template<int CACHE_SIZE>
std::mutex HashTable_phase1<CACHE_SIZE>::globalPartition;
